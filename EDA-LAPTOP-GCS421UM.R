...
#g<-aggregate(data$Tot_Benes, by=list(gender=data$Rndrng_Prvdr_Gender), FUN=sum)
...
doesthisbreak<-lmer(Tot_Benes~ 1 + (1|state), data=data)

summary(doesthisbreak)

coef(doesthisbreak)

doesthisbreak2<-lmer(Tot_Benes~(1|year), data=data)

summary(doesthisbreak2)

coef(doesthisbreak2)

doesthisbreak3<-lmer(Tot_Benes~Avg_Mdcr_Pymt_Amt+(1|state), data=data)

summary(doesthisbreak3)

coef(doesthisbreak3)

doesthisbreak4<-lmer(Tot_Benes~Avg_Mdcr_Pymt_Amt+(1|state)+(1|year), data=data)

coef(doesthisbreak4)

#test5 <- glmer(Tot_Benes ~  (1 | state), data = data, family = poisson(link = "log"))

t<-sample_n(data, 200000)

test6<-glmer(Tot_Benes~ (1|state), data=t, family=poisson(link="log"))

coef(test6)



#random sample of data and then fit on the bigger model


#could I use cost as a predictor
